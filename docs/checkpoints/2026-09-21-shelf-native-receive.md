...
- 2026-09-21，用户要求“这版记录一下，然后开始新的需求”，将001213保存为下一项需求前的基线。
- 保存标签：`we1-001213-saved-20260921`；原安装标签 `we1-001213-shelf-receive-installed-20260921` 保留。
- 用户已明确回复“授权了”；随后只读检查确认WE1屏幕／音频授权弹窗数量为0。未为此次记录重新测试拖放或操作窗口。
- 本次再次核验安装001213与备份181501的版本、源码、二进制哈希和严格签名均匹配；仅安装目录中的一个WE1主进程运行。
- 保存范围为现有代码、已安装版本、已有实测证据与下文已知限制，不等同全部功能用户验收。新需求尚待用户说明，本次仅本地保存，未推送远端。
...
- 测试中曾出现macOS屏幕／音频授权提示。助手尝试“打开系统设置”只导航，没有授予权限；之后用户自行完成授权并回复“授权了”，只读检查确认相关提示均已消失。
- 权限确认已完成；用户手动验收与顶部系统/驻留程序行为的进一步处理仍开放。本轮没有修改系统热角、系统MC设置或其他驻留软件。
