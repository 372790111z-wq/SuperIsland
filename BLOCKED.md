...
Both keyboard and pointer installation used lazy.compactMap(create).first. Pure Swift experiments call the successful factory twice, and a separate listen-only WindowServer experiment creates two registrations; invalidating the returned registration leaves the first enabled. An explicit loop creates one and cleans that one fully. Idle72071 also accumulated six disabled pointer-mask registrations and one disabled keyboard-mask registration. This is a verified creation/ownership defect; its causal contribution to native click failure still needs candidate validation.

The two production installers now share one tested sequential factory. Location order, mask, priority, callback, retained-port teardown, click policy, identity and Chrome movement are unchanged. Four allocation tests plus all prior tests pass (371 total); independent Debug build/Analyze and diff review pass. All229 frozen inputs match; candidate20260916171017 has101 verified package entries,89 unchanged resources, a stable signature and source-payload provenance. Installed162956 is unchanged at this checkpoint. Next is normal WE1 quit, recoverable actual162956 backup, installation, fresh-process tap inventory and native single-click validation. No remote push. Evidence: ../../.codex-audit/wins/2026-09-16/cmdtab-native-click-isolation/single-tap-creation/ and parent tap-creation-experiment.json.

## 2026-09-16 - candidate162956 still fails first-click acceptance; diagnosis only

User reports native Cmd-Tab app icons still require repeated clicks: first click appears to select the first/second icon, second switches. Frozen162956 records contain17 paired native clicks, all passed by the Cmd-Tab tap and none with native/preview publication inside the blocked interval. Nine settled reads remain visible after465–484ms; eight become absent after54–170ms. No anomalous Tab/Command input or long callback stall explains these click intervals. The preceding pause implementation runs as designed but is not an accepted fix; do not extend its timeout without new evidence.

The nativePlacement diagnostic combines pointer-hit and keyboard-selected sources. After a failed click, reconciliation can read the initial keyboard-selected item instead of the earlier pointer hit; this explains WE1 preview backtracking but does not prove a system selection write. Snapshot/hover paths are read-only, the panel cannot become key/main, and no commit/activation executes inside these17 native-click transactions. Existing separate preview-card commits still post their guarded Escape; do not claim the whole session contains no synthesized input.

Whole WE1 was normally quit for a second control. Process checks and1332 CG frames show no WE1 windows; WINS was not running. User answered that two-click behavior persisted. However, the captured five native-strip windows include two keyboard-release exits and three single-click exits, each disappearing before Command release; target-app activation was not observed. User report and sample are not yet matched. Do not attribute the fault to macOS or exclude WE1 globally from this evidence. A clarification about first-click strip disappearance versus wrong target remains pending.

Same installed162956 was restored asPID72071 with effective permissions and previous feature switches ON, settings closed. No production source change, new build, install, remote push or other app restart in this diagnosis. A reviewed/compiled pair of listen-only routing and CG-geometry probes now runs for a bounded900s interval, recording Command-click target/window IDs without AX calls, event injection, text or images. Zero IDs remain unknown; event timestamp/type and source metadata pair HID/annotated observations. Evidence: ../../.codex-audit/wins/2026-09-16/cmdtab-native-click-isolation/{user-two-click-0841,whole-we1-off-control,click-routing-live}/. Existing2bb25cc source and actual140527 app backup remain intact.

## 2026-09-16 - native icon single-click acceptance pending; display behavior separated

The latest user control result is “换屏，但是不需要点两次了”: Cmd-Tab Plus OFF still changes display, but native icons need only one click. Plus is restored ON; Dock display lock and other WE1 features stayed active, so this control does not exclude WE1 as a whole. Do not revert the accepted fullscreen Chrome movement fix or treat earlier lost-release evidence as the cause of this reproduction. Frozen140527 logs show preview republishing inside the unsuccessful first native click; all captured releases arrived.
...
## 2026-09-16 - candidate112637 prepared; installation and native checks pending

Current candidate20260916112637 covers Cmd-Tab tap lifecycle, fullscreen Chrome moving between physical displays, and a separately confirmed background AX self-hit crash.350 tests, independent build/Analyze, source and package checks pass. See the current first section of PROGRESS.md for scope and evidence boundaries. Installed bundle remains182704; its process crashed at11:08 and is currently stopped. Cmd-Tab Plus preference was restored ON after the native-only control. Candidate installation, Cmd-Tab release behavior, actual fullscreen cross-display movement and prior QQ close/reopen acceptance remain pending. No commit/push or business-window action occurred. Earlier entries below are historical.
